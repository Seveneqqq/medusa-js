...
export const createApprovalStep = createStep(
   ...
   async (input: any, { container }) => {
    ...
    console.log('Tutaj sie wywala 1');
      ...
      const ApprovedModuleService: any =
         container.resolve(CUSTOMER_APPROVED_MODULE);

      const approved = await ApprovedModuleService.createApproveds(input);
   ...
   async (id: string, { container }) => {
      ...
      await ApprovedModuleService.deleteApproveds(id);
   }
);